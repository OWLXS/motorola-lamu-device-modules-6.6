...
static void table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
	...
	CAM_DBG(PFX,"write 0x%x = 0x%x\n",addr, data);
	test_val = read_cmos_sensor(addr);
	CAM_DBG(PFX,"read 0x%x = 0x%x\n",addr,test_val);
	}
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < imgsensor_info.min_gain || gain > max_gain) {
		CAM_DBG(PFX,"Error max gain setting: %d\n", max_gain);
	...
	spin_unlock(&imgsensor_drv_lock);
	CAM_DBG(PFX,"gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 set_auto_flicker_mode(kal_bool enable, UINT16 framerate)
{
	...
	if (enable) /*enable auto flicker*/ {
		...
		CAM_DBG(PFX,"enable! fps = %d", framerate);
	} else {
		 /*Cancel Auto flick*/
		imgsensor.autoflicker_en = KAL_FALSE;
	}
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		CAM_DBG(PFX,"SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		CAM_DBG(PFX,"SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		CAM_DBG(PFX,"SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
